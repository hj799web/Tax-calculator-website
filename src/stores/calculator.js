...
export const useCalculatorStore = defineStore('calculator', () => {
  ...
  const capitalGainsBeforeJune25 = ref(0)
  const capitalGainsAfterJune25 = ref(0)
  const eligibleDividends = ref(0)
  const ineligibleDividends = ref(0)
  const otherIncome = ref(0)
  const rrspDeduction = ref(0)
  const maritalStatus = ref('')
  const numberOfDependents = ref(0)
  const numberOfChildrenUnder18 = ref(0)
  const numberOfDependentsWithDisabilities = ref(0)
