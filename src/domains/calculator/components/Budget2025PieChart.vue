<template>
  <div class="budget-2025-pie-chart">
...
import { budgetCategories2025 } from '@/domains/calculator/constants/taxData.js'
...
const getCategoryTranslation = (categoryKey) => {
  return t(`federalBudget.categories.y2025.${categoryKey}`) || categoryKey
}

// Compute 2025 federal budget data
const federalBudget2025DataComputed = computed(() => {
  const totalBudget = budgetCategories2025.reduce((sum, cat) => sum + cat.amount, 0)
  
  if (netFederalTaxPerPeriod.value === 0) {
    return budgetCategories2025.map((cat) => ({
...
onMounted(() => {
  console.log('Budget2025PieChart chartData:', chartData.value)
})

// Prepare chart data
const chartData = computed(() => ({
  labels: federalBudget2025DataComputed.value.map(x => x.category),
  datasets: [
    {
      label: t('federalBudget.title'),
      data: federalBudget2025DataComputed.value.map(x => x.amount),
      backgroundColor: generateColors(federalBudget2025DataComputed.value.length),
      borderWidth: 1,
    },
  ],
}))
...
const chartOptions = computed(() => ({
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: t('federalBudget.chartTitles.y2025', '2025–2026 Budget'),
      font: {
        size: 14
      }
    },
